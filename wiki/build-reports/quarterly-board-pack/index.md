...
url: "https://officeconnectpro.com/wiki/build-reports/quarterly-board-pack/"
...
- The [OfficeConnect for PowerPoint](/wiki/word-powerpoint/powerpoint/officeconnect-for-powerpoint/) add-in installed for downstream linking
...
In **D1** type `Variance`. In **D3** write `=B3-C3` and copy down. This mirrors the pattern from [Budget vs. Actuals Variance](/wiki/build-reports/budget-vs-actuals-variance/).
{{< /step >}}
...
For each tile, drop a single OfficeConnect cell that resolves to the metric for the closing quarter, and a small comparison cell underneath for the prior quarter. The full mechanics are covered in [Build a KPI Dashboard](/wiki/build-reports/kpi-dashboard/).
{{< /step >}}
...
{{< admin-note >}}
Lock all OfficeConnect cells but leave the Commentary column unlocked so reviewers can edit explanations without breaking the structure. See [Lock and Protect Reports](/wiki/build-reports/lock-protect-reports/).
{{< /admin-note >}}
...
Open your board deck in PowerPoint. Use [Publish to PowerPoint](/wiki/build-reports/publish-to-powerpoint/) or paste-as-linked-object so each refresh of the workbook flows directly into the slides. The [OfficeConnect for PowerPoint](/wiki/word-powerpoint/powerpoint/officeconnect-for-powerpoint/) add-in supports an end-to-end refresh from the slide side.
{{< /step >}}
...
- Polish the look of each tab — see [Formatted Executive Report](/wiki/build-reports/formatted-executive-report/)
- Add scenario overlays for the forward outlook — see [Scenario Comparison](/wiki/build-reports/scenario-comparison/)
- Push the final tables and KPIs into slides — see [Publish to PowerPoint](/wiki/build-reports/publish-to-powerpoint/)
